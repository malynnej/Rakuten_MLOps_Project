...
class TransformTextOutliers:
    ...
    def text_stats_calc(self, dataframe):
        ...
        print("\nWord counts:")
        print(
            "Range of word counts: ",
            stats["word_count_min"],
            " to ",
            stats["word_count_max"],
            " words.",
        )
        ...
        stats.update(
            {
                "sentence_count_min": np.min(sentence_counts),
        ...
        print(
            ...
            stats["sentence_count_min"],
            " to ",
            stats["sentence_count_max"],
            " sentences.",
        )
    ...
    def outlier_analysis(self, dataframe, factor, display_graph=False):
        ...
        print(
            "Outlier threshold: ",
            stats["word_count_mean"],
            " + ",
            factor,
            " * ",
            stats["word_count_std"],
            " = ",
    ...
    def summarizer(self, text):
        ...
        sorted_sentences = sorted(
            doc.sents,
            key=lambda sent: sum(
                word_frequencies[token.text] for token in sent if token.text in word_frequencies
            ),
            reverse=True,
        )
    ...
    def transform_outliers(self, dataframe):
        ...
        if len(violated_observation) > 0:
            df.loc[violated_observation, self.column_to_transform] = df.loc[
                violated_observation, self.column_to_transform
            ].apply(lambda row: self.cosine_similarity_calc(row))
        ...
        if len(violated_observation) > 0:
            df.loc[violated_observation, self.column_to_transform] = df.loc[
                violated_observation, self.column_to_transform
            ].apply(lambda row: self.summarizer(row))
