...
class PredictionService:
    ...
    def predict(self, 
                ...
                top_k: int = 5) -> Union[Dict, List[Dict]]:
        """
        ...
        pred_categories = [
            self.text_preparation_predict.decode_label(int(label))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file", 
        type=str, 
        help="JSON file with texts to classify (list of strings)"
    )
    parser.add_argument(
        "--designation",
        type=str,
        help="Product designation (title)"
    )
    parser.add_argument(
        "--description",
        type=str,
        default="",
        help="Product description (optional)"
    )
    parser.add_argument(
        "--top_k", 
        type=int,
        default=5,
        help="Return top-K predictions (default: 5)"
    )
    parser.add_argument(
        "--probabilities",
        action="store_true",
        help="Return probability distributions"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=32,
        help="Batch size for batch prediction (default: 32)"
    )
