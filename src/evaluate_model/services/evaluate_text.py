    ...
    def analyze_confidence(self, 
        ...
        analysis = {
            ...
            "correct_predictions": {
                ...
                "mean_confidence": float(correct_confidences.mean()) if len(correct_confidences) > 0 else 0.0,
                "median_confidence": float(np.median(correct_confidences)) if len(correct_confidences) > 0 else 0.0,
            },
            "incorrect_predictions": {
                "count": int(incorrect_mask.sum()),
                "mean_confidence": float(incorrect_confidences.mean()) if len(incorrect_confidences) > 0 else 0.0,
                "median_confidence": float(np.median(incorrect_confidences)) if len(incorrect_confidences) > 0 else 0.0,
            }
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate trained BERT model on test data"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to test.parquet (default: from config)"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
