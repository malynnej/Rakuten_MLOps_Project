...
class TestPredictSingleText:
    ...
    def test_return_probabilities(self):
        ...
        payload = {"text": self.test_text, "return_probabilities": True, "top_k": 3}
        response = requests.post(url=self.url, json=payload)
        assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"

        content = response.json()
        assert "top_predictions" in content, "Key 'top_predictions' not found in response"
...
class TestPredictProduct:
    ...
    def req(self):
        ...
        payload = {"designation": self.test_designation, "description": self.test_description}
        response = requests.post(url=self.url, json=payload)
        return response
